...
class SettingsGroup(ttk.Labelframe):
    ...
    def add_font_selector(self, parent, key):
        container = ttk.Frame(parent)
        container.pack(fill=X, expand=YES, padx=2)
        ...
        var = self.vars.get(key)
        if var:
            # EN: Combobox for font selection / CN: 字体选择下拉框
            combo = ttk.Combobox(container, textvariable=var, state="readonly")
            combo.pack(side=LEFT, fill=X, expand=YES, padx=(0, 10))
            combo.bind("<<ComboboxSelected>>", lambda e: self.on_change())
            
            # Store reference for updating list
            if key == "font_main_path": self.font_main_combo = combo
            else: self.font_sub_combo = combo
        
        return lbl

    def set_font_list(self, font_names):
        """EN: Update font list in combos / CN: 更新下拉框中的字体列表"""
        font_list = ["Default"] + font_names
        if hasattr(self, 'font_main_combo'):
            self.font_main_combo['values'] = font_list
        if hasattr(self, 'font_sub_combo'):
            self.font_sub_combo['values'] = font_list
